 ...
 */
  ...
  * @return bool
  */
 function calm_categorized_blog() {
 	if ( false === ( $all_the_cool_cats = get_transient( 'calm_categories' ) ) ) {
 		...
 		set_transient( 'calm_categories', $all_the_cool_cats );
 	}

 	if ( $all_the_cool_cats > 1 ) {
 		// This blog has more than 1 category so calm_categorized_blog should return true.
 		return true;
 	} else {
 		// This blog has only 1 category so calm_categorized_blog should return false.
 		return false;
 	}
 }

 /**
  * Flush out the transients used in calm_categorized_blog.
  */
 function calm_category_transient_flusher() {
 	if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
 		return;
 	}
 	// Like, beat it. Dig?
 	delete_transient( 'calm_categories' );
 }
 add_action( 'edit_category', 'calm_category_transient_flusher' );
 add_action( 'save_post',     'calm_category_transient_flusher' );
  ...
  */
 function calm_comment_list( $comment, $args, $depth ) {
  ...
  */
 function calm_numbered_pagination() {
 	global $wp_query;
 	$big = 999999999;
